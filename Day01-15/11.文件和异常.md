...
def main():
    f = None
    #try代码块包含可能引发异常的语句，在这个例子中，尝试打开文件并读取其中的内容
    try:
        f = open('致橡树.txt', 'r', encoding='utf-8')
        print(f.read())
    #如果在 try 代码块中引发了 FileNotFoundError 异常，程序会跳转到这个 except 代码块，然后打印一条相应的错误信息
    except FileNotFoundError:     
        print('无法打开指定的文件!')
    #如果在 try 代码块中引发了 LookupError 异常（通常是因为指定了未知的编码），程序会跳转到这个 except 代码块，然后打印相应的错误信息。 
    except LookupError:
        print('指定了未知的编码!')
    #如果在 try 代码块中引发了 UnicodeDecodeError 异常，表示在读取文件时发生解码错误，程序会跳转到这个 except 代码块，然后打印相应的错误信息
    except UnicodeDecodeError:
        print('读取文件时解码错误!')
    #无论是否发生异常，finally 代码块中的代码都会执行，在这个例子中，用于关闭文件，确保资源被正确释放。这是一个常用于清理资源的地方，例如关闭文件、关闭数据库连接等。
    finally:
        if f:
            f.close()
